...
import { isRule, isSymbolizer } from 'geostyler-style';
...
export class LegendRenderer {
     ...
     */
    renderLegendItem(output, item, position) {
        if (!this.config) {
            return;
        }
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                if (typeof olStyle == 'function') {
                    olStyle = olStyle(new OlFeature(geoms[0]), 1);
                }
     ...
     * @param {[number, number]} position the current position
     */
    renderLegend(config, output, position) {
        if (!this.config) {
            return;
        }
        output.useRoot();
     ...
     * @param {[number, number]} position the current position
     */
    async renderImages(remoteLegends, output, position) {
        ...
        for (let i = 0; i < remoteLegends.length; i++) {
            ...
            try {
                ...
                const readBlob = async (imageBlob) => {
    ...
    async renderAsImage(format, target) {
        if (!this.config) {
            return Promise.reject();
        }
        const { styles, configs, size: [width, height], remoteLegends, maxColumnWidth, maxColumnHeight, } = this.config;
        const legends = [];
        if (styles) {
     ...
     */
    async render(target, format = 'svg') {
